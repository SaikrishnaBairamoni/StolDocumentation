...
CARMADOC_BASE = Path(__file__).resolve().parents[1]
CARMADOC_BUILD = Path(args.outputdir).resolve()

# Add the '_extensions' directory to sys.path, to enable finding Sphinx
# extensions within.
sys.path.insert(0, str(CARMADOC_BASE / "doc" / "_extensions"))

# Add the '_scripts' directory to sys.path, to enable finding utility
# modules.
sys.path.insert(0, str(CARMADOC_BASE / "doc" / "_scripts"))

# Add the directory which contains the runners package as well,
# for autodoc directives on runners.xyz.
sys.path.insert(0, str(CARMADOC_BASE / "scripts" / "west_commands"))
...
with open(CARMADOC_BASE / "VERSION") as f:
    m = re.match(
        (
...
html_logo = str(CARMADOC_BASE / "doc" / "_static" / "images" / "logo.svg")
html_favicon = str(CARMADOC_BASE / "doc" / "_static" / "images" / "favicon.png")
html_static_path = [str(CARMADOC_BASE / "doc" / "_static")]
html_last_updated_fmt = "%b %d, %Y"
html_domain_indices = False
html_split_index = True
html_show_sourcelink = False
html_show_sphinx = False
html_search_scorer = str(CARMADOC_BASE / "doc" / "_static" / "js" / "scorer.js")
...
latex_elements = {
    "papersize": "a4paper",
    "maketitle": open(CARMADOC_BASE / "doc" / "_static" / "latex" / "title.tex").read(),
    "preamble": open(CARMADOC_BASE / "doc" / "_static" / "latex" / "preamble.tex").read(),
...
latex_logo = str(CARMADOC_BASE / "doc" / "_static" / "images" / "logo-latex.pdf")
latex_documents = [
...
doxyrunner_doxyfile = CARMADOC_BASE / "doc" / "zephyr.doxyfile.in"
doxyrunner_outdir = CARMADOC_BUILD / "doxygen"
doxyrunner_fmt = True
doxyrunner_fmt_vars = {"CARMADOC_BASE": str(CARMADOC_BASE), "ZEPHYR_VERSION": version}
...
warnings_filter_config = str(CARMADOC_BASE / "doc" / "known-warnings.txt")
...
kconfig_ext_paths = [CARMADOC_BASE]

# -- Options for zephyr.external_content ----------------------------------

external_content_contents = [
    (CARMADOC_BASE / "doc", "[!_]*"),
    (CARMADOC_BASE, "boards/**/*.rst"),
    (CARMADOC_BASE, "boards/**/doc"),
    (CARMADOC_BASE, "samples/**/*.rst"),
    (CARMADOC_BASE, "samples/**/doc"),
]
